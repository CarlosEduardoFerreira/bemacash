...
import static com.kaching123.tcr.store.ShopStore.ItemMatrixTable;
import static com.kaching123.tcr.store.ShopStore.VariantItemTable;
import static com.kaching123.tcr.store.ShopStore.VariantSubItemTable;
        ...
        try {
            ...
            if (c.moveToFirst()) {
                ...
                if (itemRefType == ItemRefType.Reference) {
                    List<ItemMatrixModel> itemMatrixModels = CursorUtil._wrap(ProviderAction.query(ITEM_MATRIX_URI)
                            .where(ItemMatrixTable.PARENT_GUID + "=?", itemGuid).perform(getContext()), new ItemMatrixWrapFunction());
                    ...
                    List<VariantSubItemModel> variantSubItemModels = CursorUtil._wrap(ProviderAction.query(VARIANT_SUB_ITEM_URI)
                            .where(VariantSubItemTable.ITEM_GUID + "=?", itemGuid).perform(getContext()), new VariantSubItemWrapFunction());
                    ...
                    List<VariantItemModel> variantItemModels = CursorUtil._wrap(ProviderAction.query(VARIANT_ITEM_URI)
                            .where(VariantItemTable.ITEM_GUID + "=?", itemGuid).perform(getContext()), new VariantItemWrapFunction());
