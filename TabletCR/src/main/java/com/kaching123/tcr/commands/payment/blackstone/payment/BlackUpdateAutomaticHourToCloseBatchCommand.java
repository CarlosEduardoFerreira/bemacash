 ...
 */
public class BlackUpdateAutomaticHourToCloseBatchCommand extends RESTWebCommand<AutomaticBatchCloseResponse, AutomaticBatchCloseResult> {
    ...
    protected boolean doCommand(AutomaticBatchCloseResult result) throws IOException {
        ...
        BlackStoneWebService.updateAutomaticHourToCloseBatch(request, result);

        boolean success = isResultSuccessful();
        if (!success) {
            Logger.e("BlackUpdateAutomaticHourToCloseBatchCommand.doCommand(): error result: " + (result.getData() == null ? null : result.getData().toDebugString()));
        }
