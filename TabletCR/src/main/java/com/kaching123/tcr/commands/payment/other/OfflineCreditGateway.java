...
import com.kaching123.tcr.model.payment.other.OfflineCreditTransaction;
import com.kaching123.tcr.model.payment.other.OfflineCreditTransactionFactory;
import com.telly.groundy.TaskHandler;

import java.math.BigDecimal;

public class OfflineCreditGateway implements IPaymentGateway<OfflineCreditTransaction, Void> {
    ...
    public TaskHandler refund(Context context,
        ...
        return OfflineCreditRefundCommand.start(context, callback, transaction, amount, childOrderModel, isManualReturn);
    }
    ...
    public TaskHandler voidMe(Context context,
        ...
        return OfflineCreditVoidCommand.start(context, callback, transaction, childOrderModel, needToCancel);
    }
    ...
    @Override
    public OfflineCreditTransaction createTransaction(Context context, BigDecimal amount, String orderGuid) {
        return OfflineCreditTransactionFactory.create(TcrApplication.get().getOperatorGuid(), amount, orderGuid);
    }

    @Override
    public boolean enabled() {
        return TcrApplication.get().isOfflineCreditEnabled();
    }
}
