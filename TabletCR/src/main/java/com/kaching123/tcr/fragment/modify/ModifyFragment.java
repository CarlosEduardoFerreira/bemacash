...
public class ModifyFragment extends StyledDialogFragment {
    ...
    public static void show(FragmentActivity activity, String itemGuid, int modifiersCount, int addonsCount, int optionalsCount, String defaultModifierGuid, OnAddonsChangedListener listener) {
        DialogUtil.show(activity, DIALOG_NAME, ModifyFragment_.builder()
                .numModifiers(modifiersCount)
                .numAddons(addonsCount)
                .numOptionals(optionalsCount)
                .selectedModifierGuid(new ArrayList<>(Arrays.asList(defaultModifierGuid)))
                        .itemGuid(itemGuid)
                        .build())
                .setOnAddonsChangedListener(listener);
    }

    public static void show(FragmentActivity activity, String itemGuid, int modifiersCount, int addonsCount, int optionalsCount, ArrayList<String> selectedModifierGuid, ArrayList<String> selectedAddonsGuid, ArrayList<String> selectedOptionalsGuid, OnAddonsChangedListener listener) {
        DialogUtil.show(activity, DIALOG_NAME, ModifyFragment_.builder()
                .numModifiers(modifiersCount)
                .numAddons(addonsCount)
                .numOptionals(optionalsCount)
                .itemGuid(itemGuid)
                .selectedModifierGuid(selectedModifierGuid)
                .selectedAddonsGuids(selectedAddonsGuid)
                .selectedOptionalsGuids(selectedOptionalsGuid)
                .build())
                .setOnAddonsChangedListener(listener);
    }
