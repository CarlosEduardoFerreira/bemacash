...
import com.kaching123.tcr.activity.EditReferenceItemActivity;
...
public class EditEcuadorReferenceItemActivity extends EditReferenceItemActivity {

    public static void start(Context context, ItemExModel model) {
        EditEcuadorReferenceItemActivity_.intent(context).model(model).start();
    }
    ...
    protected Loader<Cursor> onCreateTax2Loader() {
        if (TextUtils.isEmpty(model.taxGroupGuid) && !TextUtils.isEmpty(model.taxGroupGuid2)) {
            return CursorLoaderBuilder.forUri(TAX_GROUP_URI)
                    .where(ShopStore.TaxGroupTable.GUID + " = ? ", model.taxGroupGuid2)
                    .build(this);
        }
        return null;
    }
    ...
    protected Loader<Cursor> onCreateTaxLoader() {
        if (!TextUtils.isEmpty(model.taxGroupGuid)) {
            if (!TextUtils.isEmpty(model.taxGroupGuid2)) {
                return CursorLoaderBuilder.forUri(TAX_GROUP_URI)
                        .where(ShopStore.TaxGroupTable.GUID + " = ? OR " + ShopStore.TaxGroupTable.GUID + " = ?",
