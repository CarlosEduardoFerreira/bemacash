 ...
 */
public class GetMP200PaperRollSensorStatusExAction extends ActionWithAnswer<PrinterStatusEx> {
    private static int ANSWER_LEN = 5;
    private static byte[] COMMAND = new byte[]{DLE, EOT, N4};

    private final int drawerClosedValue;

    public GetMP200PaperRollSensorStatusExAction(int drawerClosedValue) {
        super(ANSWER_LEN);
    ...
    public PrinterStatusEx parseAnswer(byte[] bytes) {
        return new PrinterStatusEx(
                new PrinterStatusEx.PrinterHeadInfo(bytes[0])
        );
    }
